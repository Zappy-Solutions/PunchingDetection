...
# -------------------------------
# Frame Processing and Packet Loss Detection
# -------------------------------
frame_queue = Queue(maxsize=10)
last_valid_frame = None  # Stores the last non-white valid frame
last_frame_time = time.time()  # Track frame timestamps to detect packet loss


def read_frames(cap, queue):
    """Continuously read frames, handle white patches, and detect packet loss."""
    global last_valid_frame, last_frame_time
    ...
    while cap.isOpened():
        ...
        if not ret:
            logging.warning("[WARNING] Blank frame detected. Possible packet loss.")
            continue  # Skip blank frames

        # Detect if there's a **packet loss** (if frame delay > expected FPS interval)
        if (current_time - last_frame_time) > (1.5 / fps):  # Adjusting for network jitter
            logging.warning(
                f"[WARNING] Packet loss detected! Frame delayed by {round(current_time - last_frame_time, 2)}s")

        last_frame_time = current_time  # Update last received frame time

        # Detect fully white frames
        if np.mean(frame) > 250:
            logging.warning("[WARNING] White patch detected! Reusing last valid frame.")
            if last_valid_frame is not None:
                frame = last_valid_frame.copy()  # Use last valid frame
            else:
                continue  # Skip completely white frames if no backup exists

        # Store the last valid frame to use in case of errors
...
# -------------------------------
def main_loop():
    """Processes frames, recovers from white patches, and displays the video feed."""
    while True:

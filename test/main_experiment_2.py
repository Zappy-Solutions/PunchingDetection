...
rtsp_url = "rtsp://admin:admin123@192.168.68.115:554/rtsp/streaming?channel=01&subtype=0"
...
if mode == '1':
    ...
    logging.info(f"RTSP stream properties set to: {window_width}x{window_height} at {fps} FPS")
elif mode == '2':
...
def read_frames(cap, queue):
    ...
    while cap.isOpened():
        ...
        if not ret:
            break
        # (Optional) Resize frame if needed – here, we expect 720p already.
        queue.put(frame)
    queue.put(None)
...
def select_line_full(window_name, frame):
    ...
    def mouse_callback(event, x, y, flags, param):
        ...
        if len(points) == 2:
            y_selected = points[0][1]  # Use the y-coordinate of the first click
            points[:] = [(0, y_selected), (width, y_selected)]  # Force full-width line
            clone = frame.copy()
            cv2.line(clone, points[0], points[1], (0, 255, 0), 2)  # Draw green line
            cv2.imshow(window_name, clone)
...
logging.info("[INFO] Frame queue cleared after line selection.")
...
def main_loop():
    ...
    while True:
        ...
        # Create a named window and force it to 720p size for end-user display.
        # cv2.resizeWindow("Live CCTV Monitoring", window_width, window_height)

...
CONFIDENCE_THRESHOLD = 0.55  # Set to 0.55 (can adjust between 0.5 to 0.6)
...
# Open the webcam (Windows compatible)
...
# Let the user select the punching and crossing lines using an initial frame
ret, init_frame = cap.read()
if not ret:
    print("[ERROR] Could not read initial frame for zone selection.")
    exit()

# --- User selects the punching line ---
punching_line = select_line("Select Punching Line", init_frame.copy())
print(f"[INFO] Punching line selected: {punching_line}")

# --- User selects the crossing line ---
...
# Set a distance threshold (in pixels) to consider a point as "on" the line.
LINE_THRESHOLD = 30
...
while cap.isOpened():
    ...
    cv2.line(frame, punching_line[0], punching_line[1], (0, 255, 0), 2)  # Green line for punching
    cv2.line(frame, crossing_line[0], crossing_line[1], (0, 0, 255), 2)  # Red line for crossing
